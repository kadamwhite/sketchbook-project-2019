...
const convert = async container => {
	...
	// triangles.forEach( tri => svg.add( svg.blob( tri ) ) );

	const voronoi = d3.voronoi();
	const diagram = voronoi( points.map( p => [ p.x, p.y ] ) );
	diagram.polygons().forEach( poly => {
		svg.add( svg.shape( poly ) );
	} );
	points.forEach( point => svg.add( svg.circle( point ) ) );
	// console.log( diagram );
	// console.log( diagram.polygons() );
